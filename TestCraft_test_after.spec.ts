...
test.describe('Product interaction tests', () => {
  ...
  test('TC19: Ensure that the product description is visible and matches the product listed', async ({ page }) => {
    const description = await page.locator('.inventory_item:nth-child(1) .inventory_item_desc').textContent();
    await page.click('#item_4_title_link');
    await expect(page.locator('.inventory_details_desc')).toContainText(description);
  });
  ...
  for (let i = 1; i <= itemCount; i++) {
    const priceSelector = `.inventory_item:nth-child(${i}) .inventory_item_price`;
    const price = await page.locator(priceSelector).textContent();
...
test.describe('Menu Tests', () => {
  ...
  test('TC24: Verify that the menu is visible and accessible on the page.', async ({ page }) => {
    const menu = page.locator('.bm-burger-button');
  ...
  test('TC28: Validate that the "Reset App State" link resets the application to its default state without errors.', async ({ page }) => {
    ...
    await expect(page.locator('text=App state has been reset')).not.toBeVisible();
    const products = await page.locator('.shopping_cart_badge').count();
  ...
  test('TC29: Try to navigate to the "About" link with a slow internet connection to check if there\'s a loading indicator or timeout message.', async ({ page }) => {
    await page.click('.bm-burger-button');
    await page.route('**/*', (route) => route.continue({ delay: 10000 })); // 5 second delay
    const aboutLink = page.locator('#about_sidebar_link');
    await aboutLink.click();
...
test.describe('Shopping Cart Page', () => {
  ...
  test('TC34: Check that the cart displays the correct quantity (QTY) and description of items added by the user', async ({ page }) => {
    ...
    await page.click('.inventory_item:nth-child(1) .btn_inventory');
    await page.goto('https://www.saucedemo.com/cart.html');
    const cartQuantity = await page.locator('.cart_quantity');
    const cartDescription = await page.locator('.cart_item_label>a');
    await expect(cartQuantity).toHaveText('1');
    await expect(cartDescription).toHaveText('Sauce Labs Backpack');
  ...
  test('TC41: Attempt to navigate to the checkout page without any items in the cart and ensure an appropriate message is displayed or navigation is prevented', async ({ page }) => {
    expect(await page.textContent('#shopping_cart_container')).toBe('');
    await page.goto('https://www.saucedemo.com/cart.html');
    await page.click('#checkout');
...
test.describe('Checkout Summary Tests', () => {
  ...
  test('TC53: Confirm payment information displays correctly', async ({ page }) => {
    const paymentInfo = await page.locator('#checkout_summary_container > div > div.summary_info > div:nth-child(2)');
    await expect(paymentInfo).toContainText('SauceCard #31337');
  });

  test('TC54: Ensure shipping information is visible', async ({ page }) => {
    const shippingInfo = await page.locator('#checkout_summary_container > div > div.summary_info > div:nth-child(4)', { hasText: 'Free Pony Express Delivery!' });
