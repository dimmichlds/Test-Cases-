...
test('TC02: Add a product to the cart', async ({ page }) => {
  ...
  const cartCount = await page.textContent('#shopping_cart_container');
...
test('TC07: Filter products by category', async ({ page }) => {
  ...
  expect(products).toEqual(['Sauce Labs Backpack', 'Sauce Labs Bike Light','Sauce Labs Bolt T-Shirt','Sauce Labs Fleece Jacket','Sauce Labs Onesie','Test.allTheThings() T-Shirt (Red)' ]);
});
...
test('TC10: View product details', async ({ page }) => {
  ...
  await expect(page.locator('.inventory_details_desc')).toContainText('carry.allTheThings() with the sleek, streamlined Sly Pack that melds uncompromising style with unequaled laptop and tablet protection.');
  await expect(page.locator('.inventory_details_price')).toContainText('$29.99');
